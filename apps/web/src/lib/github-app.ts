 ...
 * (docs/specifications/onboarding-oauth.md AC-001 — GitHub App login uses the
 ...
 * of their accounts/orgs (docs/specifications/onboarding-oauth.md AC-002), not
...
/** True if `filePath` already exists on the repo's default branch. */
export async function repoFileExists(
  accessToken: string,
  { owner, repo, filePath }: { owner: string; repo: string; filePath: string },
): Promise<boolean> {
  const response = await fetch(
...
export async function featuresFolderExists(
  ...
  return repoFileExists(accessToken, { owner, repo, filePath: `${path}/GUIDELINES.md` });
}
...
/**
 * Scaffolds `docs/specifications/` on a fresh branch and opens a PR for the user
 * to merge themselves, rather than committing straight to the default
 * branch — per the risk noted in
 * docs/specifications/onboarding-oauth.md ("this needs OAuth scope wide enough
...
export async function scaffoldFeaturesFolderAsPullRequest(
  accessToken: string,
  {
    ...
    /** Full repo-relative paths, committed alongside the docs/specifications/
     * files in the same PR — e.g. the CI workflow, when it isn't already
     * present in the repo (route checks with `repoFileExists` first). */
  ...
  const allFiles = [
    ...files.map((file) => ({ path: `${path}/${file.filename}`, content: file.content })),
    ...extraFiles,
  ];
  ...
  const prResponse = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls`, {
    ...
    body: JSON.stringify({
      ...
      base: defaultBranch,
      body: [
        `Adds the canonical isidore ${path}/ guardrail files (GUIDELINES.md + templates), generated by isidore onboarding.`,
        extraFiles.length > 0
          ? `Also adds ${extraFiles.map((file) => file.path).join(", ")} — set the \`ISIDORE_HMAC_SECRET\` repo secret before merging (see the onboarding page).`
          : null,
      ]
        .filter(Boolean)
        .join("\n\n"),
